...
ip dhcp excluded-address 192.168.37.254
ip dhcp pool yyk
...
ip nat inside source static 192.168.20.1 60.0.0.11
```

说明：静态 NAT 是一对一映射，不要把多个 inside local 地址同时静态映射到同一个 inside global 地址。多个内网主机共享一个公网地址时，应使用 PAT / overload。
